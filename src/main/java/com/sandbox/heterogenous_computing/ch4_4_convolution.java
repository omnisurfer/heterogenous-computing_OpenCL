 ...
 * @author danielrowan
 */
public class ch4_4_convolution {
    
    static final float guassianBlurFilter[] = { 
    ...
    static final int guassianBlurFilterLength = 5;
        
    public static void main(String[] args) throws Exception {

        System.out.println("ch4_4_rotation()");                
               
        float convolutionFilter[] = guassianBlurFilter;
        int filterWidth = guassianBlurFilterLength;
        ...
        InputStream streamIn = ch4_4_convolution.class.getResourceAsStream("/Convolution.cl");              
        ...
        CLBuffer<FloatBuffer> clConvolutionBuffer = context.createFloatBuffer(convolutionFilter.length, READ_ONLY);
                        
        fillBuffer(clConvolutionBuffer.getBuffer(), convolutionFilter);
         
        clConvolutionBuffer.getBuffer().rewind();
        
        /*
        int tempSize = (int)clConvolutionBuffer.getCLCapacity();                
        
        float tempBuff[] = new float[tempSize];
        
        FloatBuffer floatBuffer = clConvolutionBuffer.getBuffer();
        
        floatBuffer.rewind();
        
        floatBuffer.get(tempBuff);       
        */
        //
        ...
        //map buffers to kernel function 'Convolution'
        CLKernel kernel = program.createCLKernel("Convolution");
        kernel.putArgs(inputImageMemory, outputImageMemory, clConvolutionBuffer)
        ...
        try {
            queue.putWriteImage(inputImageMemory, true)
                .putWriteBuffer(clConvolutionBuffer, true)
                .put2DRangeKernel(
                        kernel, 0, 0, inputBufferedImage.getWidth(), inputBufferedImage.getHeight(), 0, 0);
        ...
        System.out.println("Write out convolved image: ");
        ...
        try (FileOutputStream fos = new FileOutputStream("Resources/Images/convolution.bmp")) {
        ...
        if(!inputImageMemory.isReleased())
            inputImageMemory.release();
        
        if(!outputImageMemory.isReleased())
            outputImageMemory.release();
    ...
    private static void fillBuffer(FloatBuffer buffer, float[] input)
    {
        int bufferDiff = buffer.capacity() - input.length;
        int i = 0;
        while(buffer.remaining() != bufferDiff)
        ...
        float temp[] = new float[buffer.capacity()];
